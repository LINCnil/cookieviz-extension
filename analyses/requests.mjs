    ...
    return new Promise((resolve, reject) => {
        ...
        getRequest.onsuccess = function () {
            ...
            let filtered_links = [...new Set(getRequest.result
                .filter(x => x.cookie && x.url && x.referer)
                .map(x => JSON.stringify(x)))].map(x => JSON.parse(x));

            filtered_links
                .forEach(x => {
                    const source = getBaseDomain(x.referer);
...
const requests = {
    ...
    data: {
        ...
        "nodes_requests": getAllNodes.bind(null, "request_table", ["page", "referer", "url"], false),
        "nodes_requests_with_cookies": getAllNodes.bind(null, "request_table", ["page", "referer", "url"], true),
